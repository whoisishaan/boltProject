...
const ThemeToggle: React.FC = () => {
  ...
  return (
    <button
      onClick={toggleTheme}
      className={`
        fixed top-7 right-20 z-50 p-3 rounded-2xl border transition-all duration-300 group
        ${isDark 
    ...
    >
      ...
      <div className="relative">
        {isDark ? (
          <Sun className="w-6 h-6 text-yellow-400 group-hover:text-yellow-300 transition-colors duration-200" />
        ) : (
          <Moon className="w-6 h-6 text-gray-700 group-hover:text-blue-600 transition-colors duration-200" />
        )}
      </div>
